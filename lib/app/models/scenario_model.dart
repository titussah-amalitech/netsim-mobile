...
import 'package:netsim_mobile/app/models/device_metadata.dart';
import 'package:netsim_mobile/app/models/device_position_model.dart';
...
class ScenarioModel {
  ...
  factory ScenarioModel.fromMap(Map<String, dynamic> map) {
    ...
    int timeLimitVal = parseInt(map['timeLimit'] ?? map['time_limit'] ?? map['time']);

    List<Device> devicesVal = [];
    try {
      final rawDevices = map['devices'] ?? map['device_list'];
      if (rawDevices is List) {
        devicesVal = rawDevices.map((x) {
          try {
            ...
            if (x is Map<String, dynamic>) {
              ...
              if (x.containsKey('device')) {
                ...
                if (dev is Map) {
                  ...
                  if (x.containsKey('position')) merged['position'] = x['position'];
                  if (x.containsKey('parameters')) merged['parameters'] = x['parameters'];
                  if (x.containsKey('status')) merged['status'] = x['status'];
                  // ensure id fields use common keys
                  if (merged.containsKey('_id') && !merged.containsKey('id')) merged['id'] = merged['_id'];
                  try {
                  ...
                  try {
                    ...
                    try {
                      ...
                      positionVal = rawPos is Map<String, dynamic>
                          ...
                          : DevicePosition.fromMap(Map<String, dynamic>.from(rawPos));
                    } catch (_) {
                    ...
                    try {
                      final rawParams = x['parameters'] ?? <String, dynamic>{};
                      parametersVal = rawParams is Map<String, dynamic>
                          ? DeviceParameters.fromMap(rawParams)
                          : DeviceParameters.fromMap(Map<String, dynamic>.from(rawParams));
                    } catch (_) {
                    ...
                    try {
                      ...
                      statusVal = rawStatus is Map<String, dynamic>
                          ...
                          : DeviceStatus.fromMap(Map<String, dynamic>.from(rawStatus));
                    } catch (_) {
            ...
            if (x is Map) return Device.fromMap(Map<String, dynamic>.from(x));
          } catch (_) {
    ...
    try {
      ...
      metadataVal = rawMeta is Map<String, dynamic> ? Metadata.fromMap(rawMeta) : Metadata.fromMap(Map<String, dynamic>.from(rawMeta));
    } catch (_) {
      metadataVal = Metadata(createdBy: '', createdAt: DateTime.now(), description: '');
    }
